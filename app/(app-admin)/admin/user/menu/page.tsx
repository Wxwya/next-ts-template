...
const MenuPage = () => {
  ...
  const columns: TableColumns<Menu.MenuInfo>[] = [
    ...
    },
    {
      ...
      cell: (row) => (<div className='flex items-center gap-2'>
        ...
        <Link href={`/admin/user/subMenu/${row.id}/${row.path}`} data-auth='xwya:menu:sub' className=' text-blue-500 cursor-pointer' >子菜单管理</Link>
  ...
  const getData = async (newPage?: PageType) => {
    setLoading(true)
    const res = await getMenuList({ ...page,...newPage,...queryBody,})
    if (res.code===200) { 
  ...
  const onOpen = (row?:Menu.MenuInfo) => { 
    const close = createModal({
      title: row ? "编辑菜单" : "新增菜单",
      children: <MenuModal row={row} onClose={() => close?.()}  getData={getData} total={total}  />,
      footer: false,
    })
  }
