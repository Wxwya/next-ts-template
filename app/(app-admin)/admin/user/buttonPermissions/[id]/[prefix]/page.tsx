...
const SubPermissions = () => {
  ...
  const getData = async (newPage?: PageType) => {
    setLoading(true)
    const res = await getPermissionsList({ ...page,...newPage,...queryBody,parent_id,})
    if (res.code===200) { 
  ...
  const onOpen = (row?:Permissions.PermissionsInfo) => { 
    const close = createModal({
      title: row ? "编辑按钮权限" : "新增按钮权限",
      children: <PermissionModal row={row} prefix={prefix as string} onClose={() => close?.()} parent_id={parent_id as string}  getData={getData}  />,
      footer: false,
    })
  }
    ...
    <XwyaForm labelWidth={60} labelAlign="right" items={items} form={form} onFinish={onFinish} row={5} >
      {
        <div className=" flex gap-2">
          ...
          <XwyaButton data-auth="xwya:permission:add" tooltipText="新增按钮权限" className=" bg-blue-500 hover:bg-blue-600" type="button" onClick={() => onOpen()} icon="solar--add-circle-linear" />
