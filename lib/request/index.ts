...
const requestHooks: RequestHooks = {
  async beforeRequest(options: RequestOptions | RequestSendBodyOptions, config: RequestConfig): Promise<RequestSendBodyOptions> {
    ...
    if (newOptions.withToken) {
      ...
      if (newOptions.stateRefresh) {
        ...
        newOptions.refreshTokenKey &&
          ((newOptions!.headers as Record<string, any>)[newOptions.refreshTokenKey!] = `${newOptions?.tokenPrefix ?? ''}${refreshToken}`)
      }
    }
  ...
  async afterRequest<T>(data: DecryptBody<T>, config: RequestConfig, response: Response): Promise<DecryptBody<T>> {
    ...
    if (response && response.status != RequestCodeEnum.SUCCESS) {
      flag && toast.error(data ? `${data.code}:${JSON.stringify(data.msg)}` : `${response.status}:${response.statusText}`)
    ...
    if (data) {
      if (data.code === RequestCodeEnum.SUCCESS) {
        flag && config?.show && toast.success(config.message || data.msg || 'ok')
      } else if (RequestCodeEnum.TOKEN_INVALID.includes(data.code)) {
        if (!that.isRefreshing && that.options.stateRefresh && that.options.withToken && !config?.isAuth && data.code===RequestCodeEnum.TOKEN_TIMEOUT && flag) {
        ...
        if ((!that.isRefreshing || config?.isAuth) && flag ) {
          cache.clearCookie()
          toast.error(data.msg || '暂无权限..')
          location.reload()
        }
        return { code: data.code, msg: data.msg, data: null }
      } else if (data.code === RequestCodeEnum.SERVER_ERROR) {
        flag && toast.error('请稍后重试....')
        console.error('\x1b[31m%s\x1b[0m', `❌ 错误：${data.msg || '请求错误'}`)
      } else {
        flag && toast.error(data.msg || '请求错误')
        console.error('\x1b[31m%s\x1b[0m', `❌ 请求返回错误：${data.msg || '请求错误'}`)
      }
    }
    if ((!config?.isAuth && that.options.stateRefresh) || !flag) {
      that.removeTask()
    }
    return data
  },
